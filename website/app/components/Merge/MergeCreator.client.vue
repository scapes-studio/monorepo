  ...
  <section class="merge-creator">
    ...
    <div class="merge-creator__canvas grid-shadow">
      ...
      <template v-else-if="scapes.length > 0">
        <img
          v-if="previewUrl && scapes.length >= 2"
        ...
        <ScapeImage
          v-else
          :id="scapes[0]![0]"
          class="merge-creator__image-single"
        />
        <p
          v-if="scapes.length === 1"
          class="muted"
        >
          Select another scape...
        </p>

        <Actions
          v-if="scapes.length >= 2"
          class="scape-actions"
        >
          <div
      ...
      >
        <template #start="{ start }">
          ...
          >
            {{
              canMerge
                ? `Merge ${scapes.length} Scapes`
                : 'Select at least 2 Scapes'
            }}
          </Button>
        </template>
    ...
    <Loading
      v-if="loading && displayedScapes.length === 0"
      txt="Loading scapes..."
    />
    <div
      v-else-if="scapeError"
      class="merge-creator__status merge-creator__status--error"
    >
      Unable to load scapes right now.
    </div>
    <div
      v-else-if="displayedScapes.length === 0"
      class="merge-creator__status"
    >
      {{
        searchedId
          ? `Scape #${searchedId} is already selected.`
          : 'No Scapes available.'
      }}
    </div>
    ...
    <ScapesGrid
      v-else
      :scapes="displayedScapes"
      :columns="contentColumns"
    >
      <template #item="{ scape, scapeCount }">
        <button
          type="button"
          class="merge-creator__scape-btn"
          :disabled="isFull"
    ...
    <Button
      v-if="hasMore && !searchedId && displayedScapes.length > 0"
      ref="loadMoreRef"
      class="merge-creator__load-more small tertiary"
      :disabled="loading"
      @click="loadMore"
    >
...
const availableScapes = computed(() => {
  const source = onlyOwned.value
    ? ownerData.scapes.value
    : galleryData.scapes.value
  return source.filter(
    (s) => s.id <= 10000n && !selectedIds.value.includes(s.id),
  )
})
...
.merge-creator__canvas {
  ...
  width: 100%;
  min-height: calc(
    var(--scape-height-gutter) * var(--content-columns) - var(--grid-gutter)
  );
  padding: var(--spacer);
  margin: 0;
