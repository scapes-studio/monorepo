<template>
  <li class="history-item">
    <GridArea
      :rows="1"
      width="full"
      class="history-item__header no-shadow"
    >
      <span class="history-item__type">
        ...
        <template v-if="entry.type === 'sale' && price">
          <span class="muted">
            ...
            <span
              v-if="entry.sale?.source"
              class="history-item__source"
            >
              via {{ entry.sale.source }} </span
            >)
          </span>
        </template>
    ...
    <GridArea
      v-if="entry.type === 'listing' || !isSingleLine(entry)"
      :rows="1"
      width="full"
...
const isMigration = (entry: TransferEntry) =>
  entry.from.toLowerCase() === ZERO_ADDRESS &&
  entry.timestamp > MIGRATION_CUTOFF
...
const isMerge = (entry: TransferEntry) =>
  entry.from.toLowerCase() === ZERO_ADDRESS &&
  BigInt(props.scapeId || 0) > 10_000n

const isSingleLine = (entry: TransferEntry) =>
  isMerge(entry) || isMigration(entry)
