<template>
  <GridArea v-if="isActive || canClaim || canWithdraw" class="gallery27-actions" rows="2" padding>
    <header>
    ...
    <Gallery27ActionWithdraw v-else-if="canWithdraw" :punk-scape-id="punkScapeId"
      :latest-bid="auction!.latestBid!" :contract-address="auctionContractAddress!"
...
const canWithdraw = computed(() => {
  return props.hasEnded && !props.isActive && isPunkScapeOwner.value && hasBids.value && rewardsClaimed.value === false;
});
