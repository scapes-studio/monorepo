  ...
  <template v-else>
    ...
    >
      ...
      <span v-if="auctionStatus === 'not-started'">
        <NuxtLink :to="`/${scapeId}`"> #{{ scapeId }} </NuxtLink>
      </span>
      <span v-else>
        <NuxtLink
          :to="`/${scapeId}`"
    ...
    <GridArea
      v-if="auctionStatus === 'not-started' && auction.startTimestamp"
      class="gallery27-meta"
      padding
    >
      <span>Starts in</span>
    ...
    <GridArea
      v-if="auctionStatus === 'active' && auction.endTimestamp"
      class="gallery27-meta"
      padding
    >
      <span>Ends in</span>
    ...
    <GridArea
      v-if="auctionStatus !== 'not-started'"
      class="gallery27-meta"
      padding
    >
      <span>Bids</span>
    ...
    <GridArea
      v-else-if="auction.latestBidder"
      class="gallery27-meta"
      padding
    >
      <span>{{
...
const ownerDiffersFromWinner = computed(() => {
  ...
  if (auctionStatus.value !== 'ended' && auctionStatus.value !== 'settled')
    return false
