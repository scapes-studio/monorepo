...
body {
  ...
  padding-bottom: calc(
    var(--grid-padding) + var(--scape-height-gutter) +
      var(--scape-height-gutter)
  );
  background-image:
    linear-gradient(
      to right,
      var(--grid-color) var(--grid-gutter),
  ...
  background-size: calc(var(--scape-width) + var(--grid-gutter))
    calc(var(--scape-height) + var(--grid-gutter));
  background-position: var(--grid-margin-offset, 0px)
    var(--grid-margin-offset, 0px);
  opacity: 0;
