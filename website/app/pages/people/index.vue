<template>
  <section class="accounts-page">
    ...
    <div
      v-if="pending"
      class="accounts-page__status"
    >
      Loading owners…
    </div>
    <div
      v-else-if="error"
      class="accounts-page__status accounts-page__status--error"
    >
    ...
    <div
      v-else
      class="accounts-page__results"
    >
      <ol class="accounts-page__list">
        <li
          v-for="(entry, index) in owners"
          :key="entry.owner"
          class="accounts-page__row grid-shadow"
        >
          <span class="accounts-page__rank">{{ index + 1 }}</span>
          <AccountLink
            :address="entry.owner"
            class="accounts-page__owner"
            shorten-ens
          />
      ...
      <button
        v-if="hasMore"
        ref="loadMoreRef"
        class="accounts-page__load-more"
        type="button"
...
defineOgImageComponent('PeopleIndex', {
  title: ogTitle,
  subtitle: ogSubtitle,
  image: ogImage,
})
...
const {
  public: { scapeCollectionAddress },
} = useRuntimeConfig()
const normalizedCollectionAddress =
  scapeCollectionAddress.toLowerCase() as `0x${string}`
...
const fetchOwners = async (startOffset: number) =>
  client.db
    .select({
    ...
    .orderBy(desc(count), asc(schema.scape.owner))
    .limit(PAGE_SIZE)
    .offset(startOffset)
...
const {
  ...
  pending,
  error,
} = await useAsyncData('scape-owners-leaderboard', () => fetchOwners(0))
const { data: totalsData } = await useAsyncData(
  'scape-owners-totals',
  async () => {
    const ownersCount = await client.db
      .select({
        total: sql<number>`count(distinct ${schema.scape.owner})::int`,
      })
      .from(schema.scape)
      .where(excludeScapeCollection)
