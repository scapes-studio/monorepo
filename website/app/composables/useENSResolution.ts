...
export const useAccountDisplay = (
  address: MaybeRefOrGetter<`0x${string}` | string | undefined>,
) => {
  const runtimeConfig = useRuntimeConfig()
  const addressValue = computed(() => toValue(address))

  const asyncKey = computed(
    () => `ens-${addressValue.value?.toLowerCase() ?? 'unknown'}`,
  )

  const { data: profile } = useAsyncData(asyncKey, async () => {
    if (!addressValue.value) return null
    const baseUrl = runtimeConfig.public.apiUrl.replace(/\/$/, '')
    try {
      return await $fetch<ProfileResponse>(
        `${baseUrl}/profiles/${addressValue.value}`,
      )
    } catch {
      return null
    }
...
export const useENSResolution = () => {
  ...
  const resolveAddress = async (address: string): Promise<void> => {
    ...
    const request = $fetch<ProfileResponse>(
      `${baseUrl.value}/profiles/${address}`,
    )
      .then((response) => {
