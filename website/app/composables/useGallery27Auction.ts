...
export const useGallery27Auction = (
  tokenId: Ref<string | number | undefined>,
) => {
  const runtimeConfig = useRuntimeConfig()

  const asyncKey = computed(
    () => `gallery27-auction-${tokenId.value ?? 'unknown'}`,
  )
  ...
  const { data, pending, error, refresh } = useAsyncData(
    ...
    async () => {
      ...
      return await $fetch<Gallery27AuctionState>(
        `${baseUrl}/gallery27/${tokenId.value}/auction`,
      )
    },
    { watch: [tokenId] },
  )
  ...
  const isActive = computed(() => {
    ...
    if (data.value.startTimestamp && now.value < data.value.startTimestamp)
      return false
