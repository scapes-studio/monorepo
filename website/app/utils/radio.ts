...
export const randomScape = (): number => randomBetween(MIN_SCAPE_ID, MAX_SCAPE_ID);

// Formula for synchronized live radio (same scape plays for all users at same time)
export const liveRadioScape = (): number =>
  Math.floor((Math.floor((Date.now() / 1000) / 64) * 444444443) % MAX_SCAPE_ID + 1);

export const nextLiveRadioScape = (): number =>
  Math.floor((Math.floor((Date.now() / 1000 + 64) / 64) * 444444443) % MAX_SCAPE_ID + 1);
